...
class LoginManager {
	...
	public static function replace_magic_link_in_wp_mail( $atts ) {
		...
		// Normalize encoded placeholders
		$atts['message'] = strtr(
			$atts['message'],
			[
				'%7B%7BMAGIC_LINK%7D%7D'         => '{{MAGIC_LINK}}',
				'%7B%7BMAGIC_LOGIN_QR%7D%7D'     => '{{MAGIC_LOGIN_QR}}',
				'%7B%7BMAGIC_LOGIN_QR_IMG%7D%7D' => '{{MAGIC_LOGIN_QR_IMG}}',
			]
		);

		$has_link       = strpos( $atts['message'], '{{MAGIC_LINK}}' ) !== false;
		$has_qr_src     = strpos( $atts['message'], '{{MAGIC_LOGIN_QR}}' ) !== false;
		$has_qr_img_tag = strpos( $atts['message'], '{{MAGIC_LOGIN_QR_IMG}}' ) !== false;

		if ( ! $has_link && ! $has_qr_src && ! $has_qr_img_tag ) {
			return $atts;
		}
		...
		if ( self::is_single_recipient( $atts ) ) {
			...
			if ( $user ) {
				...
				if ( $replace_magic_link ) {
					...
					if ( $has_qr_src ) {
						$qr_src = \MagicLogin\QR::get_image_src( $magic_link );
						/**
						 * Filter qr image source for wp_mail integration {{MAGIC_LOGIN_QR}}
						 *
						 * @hook  magic_login_replace_magic_login_qr_in_wp_mail
						 *
						 * @param string   $qr_src     qr image source
						 * @param string   $magic_link magic link
						 * @param array    $atts       wp_mail args
						 * @param \WP_User $user       user object
						 *
						 * @return string Altered qr image source
						 * @since 2.5
						 */
						$qr_src = apply_filters( 'magic_login_replace_magic_login_qr_in_wp_mail_message', $qr_src, $magic_link, $atts, $user );
					}

					if ( $has_qr_img_tag ) {
						$qr_img_tag = \MagicLogin\QR::get_img_tag( $magic_link );
						/**
						 * Filter qr image tag for wp_mail integration {{MAGIC_LOGIN_QR_IMG}}
						 *
						 * @hook  magic_login_replace_magic_login_qr_img_in_wp_mail_message
						 *
						 * @param string   $qr_img_tag qr image tag
						 * @param string   $magic_link magic link
						 * @param array    $atts       wp_mail args
						 * @param \WP_User $user       user object
						 *
						 * @return string Altered qr image tag
						 * @since 2.5
						 */
						$qr_img_tag = apply_filters( 'magic_login_replace_magic_login_qr_img_in_wp_mail_message', $qr_img_tag, $magic_link, $atts, $user );
					}
				}
			}
		}
		...
		$atts['message'] = strtr(
			...
			[
				...
				'{{MAGIC_LOGIN_QR}}'     => $qr_src,
				'{{MAGIC_LOGIN_QR_IMG}}' => $qr_img_tag,
			]
		);
	...
	public static function ajax_request() {
		...
		$global_data = [ 'log', 'redirect_to', 'g-recaptcha-response', 'magic-login-cf-turnstile-response' ];
	...
	public static function is_auto_login_link_excluded_mail( $args ) {
		 ...
		 *
		 * @hook magic_login_auto_login_excluded_subjects
		 *
		 * @param array $excluded_subjects
		 *
		 * @return array Altered excluded subjects
	...
	public static function handle_login_request() {
		...
		nocache_headers();
		/**
		 * Fires before handling the login request
		 *
		 * @hook magic_login_handle_login_request
		 */
	 ...
	 *
	 * @return void|string|array Returns error message or redirect info if $code_login is true.
	 * @since 2.4
	 */
	public static function authenticate_user_token( $user_id, $token, $code_login = false ) {
		...
		if ( ! $user ) {
			/**
			 * Fires when the user is not found.
			 *
			 * @hook magic_login_invalid_user
			 */
			 ...
			 *
			 * @hook magic_login_error_message
			 *
		...
		// Validate token.
		$token_validity = \MagicLogin\Utils\get_token_validity_by_user( $user->ID );
		...
		if ( ! $is_valid ) {
			/**
			 * Fires when the token is invalid.
			 *
			 * @hook magic_login_invalid_token
			 */
			do_action( 'magic_login_invalid_token' );
			/**
			 * Filter the error message for invalid token.
			 *
			 * @hook magic_login_invalid_token_error_message
			 *
			 * @param string $error Default error message.
			 *
			 * @return string Modified error message.
		 ...
		 *
		 * @hook magic_login_before_login
		 *
		 * @param \WP_User $user          The WP_User object.
		 * @param array    $current_token The current token data.
		 */
		...
		/**
		 * Fires after the user is logged in.
		 *
		 * @hook magic_login_logged_in
		 *
		 * @param \WP_User $user          The WP_User object.
		 * @param array    $current_token The current token data.
		 */
		...
		/**
		 * Fires after the user is logged in as standard WordPress action.
		 *
		 * @hook wp_login
		 *
		...
		/**
		 * Filter the redirect URL for Magic Login.
		 *
		 * @hook magic_login_redirect
		 *
		 * @param string   $redirect_to The redirect URL.
		 * @param \WP_User $user        The WP_User object.
		 *                              * @return string The modified redirect URL.
	...
	public static function process_login_request( $args = [] ) {
		...
		/**
		 * Filter the info message
		 *
		 * @hook magic_login_info_message
		 *
		 * @param string $info Default info message.
		 * @param array  $args Arguments passed to the function.
		 *
		 * @return string Modified info message.
		 */
	...
	public static function maybe_redirect() {
		 ...
		 *
		 * @hook magic_login_before_login_form_redirect
		 *
	...
	public static function process_login_form_submission( $args ) {
		...
		/**
		 * Fires before processing the login request
		 *
		 * @hook magic_login_pre_process_login_request
		 *
		 * @param null|\WP_Error $result Result of the filter
		 *
		...
		/**
		 * Apply pre-send link filter. Useful for preventing sending the login link.
		 *
		 * @hook magic_login_pre_send_login_link
		 *
		 * @param null|\WP_Error $send_link
		 * @param \WP_User       $user
		 *
		 * @return null|mixed Altered value.
		 */
		...
		if ( ! is_a( $user, '\WP_User' ) ) {
			...
			if ( $settings['registration']['enable'] ) {
				if ( 'standard' === $settings['registration']['mode'] ) {
					$show_registration_form = true;
				}

				if ( 'auto' === $settings['registration']['mode'] ) {
					if ( is_email( $user_name ) ) {
						...
						$new_user_id = \MagicLogin\Registration::register_user( $userdata );

						if ( ! is_wp_error( $new_user_id ) ) {
							$errors = null;
							if ( ! $settings['registration']['send_email'] ) {
								$user   = get_user_by( 'id', $new_user_id );
								$errors = self::send_login_link( $user );
							}
						}
					} elseif ( $settings['registration']['fallback_email_field'] ) {
						$show_registration_form = true;
					}
				}
			}
			...
			if ( ! empty( $magic_login_code_login_result ) ) {
				...
				if ( SmsService::is_sms_login_enabled() ) {
					if ( 'phone_only' === $settings['sms']['sms_sending_strategy'] ) {
						if ( $phone_login ) {
							$errors = SmsService::send_login_sms( $user->ID );
						} else {
							$errors = self::send_login_link( $user, false, $code_login );
						}
					} elseif ( 'sms_and_email' === $settings['sms']['sms_sending_strategy'] ) {
						SmsService::send_login_sms( $user->ID );
						$errors = self::send_login_link( $user, $magic_login_link ); // send the same code/link to email
					} elseif ( 'sms_or_email' === $settings['sms']['sms_sending_strategy'] ) {
						$status = SmsService::send_login_sms( $user->ID );
						if ( ! $status ) {
							$errors = self::send_login_link( $user, false, $code_login );
						}
					}
		...
		if ( ! is_wp_error( $errors ) ) {
			...
			if ( $code_login ) {
				...
				if ( $phone_login ) {
					$info = '<p class="message magic_login_block_login_success">' . __( 'Please enter the code sent to your phone number or email.', 'magic-login' ) . '</p>';
				}
	...
	public static function send_login_link( $user, $login_link = false, $code_login = false ) {
		...
		$settings      = \MagicLogin\Utils\get_settings();
		$login_email   = __( $settings['login_email'], 'magic-login' ); // phpcs:ignore
		$email_subject = __( $settings['email_subject'], 'magic-login' ); // phpcs:ignore
		 ...
		 *
		 * @hook magic_login_email_content
		 *
		 * @param string $login_email        Email content
		 ...
		 *
		 * @hoook magic_login_email_subject
		 *
		...
		/**
		 * Fires before sending the login link email.
		 *
		 * @hook magic_login_send_login_link
		 *
